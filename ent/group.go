...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.ForeignKeys[0]: // user_admin_of
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case group.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_admin_of", value)
			} else if value.Valid {
				gr.user_admin_of = new(int)
				*gr.user_admin_of = int(value.Int64)
			}
		}
	}
	return nil
